...
def run_probe_start(
    *,
    ...
    probe_runner_descriptor: Path | None,
    ...
    if evidence_mode == EvidenceDeploymentMode.LOCAL:
        ...
        if any(value is not None for value in remote_options):
            raise ValueError("local evidence mode does not accept remote probe_runner options")
        ...
        if allow_executable:
            raise ValueError("remote executable allowlists belong to the target probe_runner")
        enrollment_options = (probe_runner_descriptor, verification_secret, ssh_target, known_hosts)
        if deployment_path.is_file() and not any(value is not None for value in enrollment_options):
            ...
            if not all(value is not None for value in enrollment_options):
                raise ValueError(
                    "remote evidence mode requires an enrolled deployment or all probe_runner options"
                )
            descriptor = ProbeRunnerDescriptor.model_validate_json(
                probe_runner_descriptor.read_text(encoding="utf-8")
            )
            deployment = configure_deployment(
                ...
                probe_runner_executable=probe_runner_executable or "robotctl",
            )
        request = new_request(robot_id)
    ...
    # RKB-4 publishes the verified snapshot and metadata-only Episode as the
    # canonical artifact path for the v4 evidence contract.
    from rolo.rkb import publish_probe_episode

    bundle_ref = f"artifact://target-evidence/{bundle.robot_id}-bundle.json"
...
def probe_stage_start(
    ...
    probe_runner_descriptor: Annotated[Path | None, typer.Option("--probe-runner-descriptor")] = None,
    ...
    probe_runner_config: Annotated[
        str, typer.Option("--probe-runner-config")
    ...
    try:
        emit(
            run_probe_start(
                ...
                probe_runner_descriptor=probe_runner_descriptor,
