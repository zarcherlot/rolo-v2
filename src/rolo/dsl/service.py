...
class RoloDslCompiler:
    def check(self, request: DslCheckRequest) -> DslCompileResult:
        ...
        if document is None: return DslCompileResult(status="DSL_CHECK_FAILED", dsl_digest="", diagnostics=tuple(item.code for item in report.diagnostics))
    ...
    def compile(self, request: DslCompileRequest, output_dir: str | Path) -> DslCompileResult:
        document, report = parse_document(request.dsl)
        if document is None: return DslCompileResult(status="DSL_COMPILE_FAILED", dsl_digest=request.dsl_digest, diagnostics=tuple(item.code for item in report.diagnostics))
