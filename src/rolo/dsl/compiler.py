...
class CompileResult:
    def __init__(self, *, document, ir, bundle, report, dsl_digest): self.document, self.ir, self.bundle, self.report, self.dsl_digest = document, ir, bundle, report, dsl_digest
    @property
    def ok(self): return self.report.ok and self.bundle is not None
def compile_document(document: DslDocument, output_dir: str | Path, context: dict | None = None) -> CompileResult:
    ir, report, digest = compile_frontend(document)
    if context is not None and report.ok: report = resolve_evidence(document, context)
...
def compile_text(value, output_dir: str | Path, context: dict | None = None) -> CompileResult:
    ...
    if document is None: return CompileResult(document=None, ir=None, bundle=None, report=report, dsl_digest="")
