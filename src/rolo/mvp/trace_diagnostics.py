...
def build_odom_ekf_diagnostic_plan() -> TraceDiagnosticPlan:
    ...
    steps = (
        TraceDiagnosticStep(step_id="PROCESS_PRESENCE", title="bringup/odom/EKF presence", argv=("ros2", "node", "list", "--no-daemon"), timeout_s=10, pass_condition="bringup, odom_publisher and EKF are present", block_code="PROCESS_MISSING"),
        TraceDiagnosticStep(step_id="TOPIC_GRAPH", title="EKF publisher/subscriber graph", argv=("ros2", "topic", "info", "/odom", "-v"), timeout_s=10, pass_condition="/odom has the expected EKF publisher and subscribers", block_code="GRAPH_INCOMPLETE"),
        TraceDiagnosticStep(step_id="ODOM_RAW_GRAPH", title="raw odometry graph", argv=("ros2", "topic", "info", "/odom_raw", "-v"), timeout_s=10, pass_condition="/odom_raw has the expected raw odometry publisher", block_code="RAW_GRAPH_INCOMPLETE"),
        TraceDiagnosticStep(step_id="ODOM_RAW_SAMPLE", title="raw odometry receipt", argv=("ros2", "topic", "echo", "/odom_raw", "--once"), timeout_s=8, pass_condition="at least one live /odom_raw sample is received", block_code="ODOM_RAW_SILENT"),
        TraceDiagnosticStep(step_id="EKF_SAMPLE", title="EKF odometry receipt", argv=("ros2", "topic", "echo", "/odom", "--once"), timeout_s=8, pass_condition="at least one live /odom sample is received", block_code="EKF_SILENT"),
        TraceDiagnosticStep(step_id="IMU_SAMPLE", title="IMU receipt", argv=("ros2", "topic", "echo", "/imu", "--once"), timeout_s=8, pass_condition="at least one live /imu sample is received", block_code="IMU_SILENT"),
        TraceDiagnosticStep(step_id="TF_CHAIN", title="odom to base transform", argv=("ros2", "run", "tf2_ros", "tf2_echo", "odom", "base_footprint"), timeout_s=8, pass_condition="odom→base_footprint transform is available", block_code="TF_MISSING"),
        TraceDiagnosticStep(step_id="COMMAND_EXCLUSIVITY", title="autonomous cmd_vel source", argv=("ros2", "topic", "info", "/controller/cmd_vel", "-v"), timeout_s=8, pass_condition="the designated autonomous command source is confirmed and graph counts are recorded", block_code="COMMAND_SOURCE_UNVERIFIED"),
        TraceDiagnosticStep(step_id="IDENTITY_CONSISTENCY", title="executor identity consistency", argv=("id", "-un"), timeout_s=5, pass_condition="diagnostic executor uses the same OS user as ROS publishers", block_code="DDS_USER_MISMATCH"),
        TraceDiagnosticStep(step_id="SENSOR_SEMANTICS", title="EKF and IMU measurement semantics", argv=("ros2", "param", "dump", "/ekf_filter_node"), timeout_s=10, pass_condition="the EKF input configuration and IMU yaw observability can be identified", block_code="SENSOR_SEMANTICS_UNKNOWN"),
        TraceDiagnosticStep(step_id="INITIAL_HEADING", title="initial heading evidence", argv=("ros2", "topic", "echo", "/imu", "--once"), timeout_s=8, pass_condition="an IMU sample is captured so its orientation validity and startup heading contract can be assessed", block_code="IMU_YAW_UNOBSERVABLE"),
        TraceDiagnosticStep(step_id="RESET_SURVIVAL", title="odom reset topic contract", argv=("ros2", "topic", "info", "/set_odom", "-v"), timeout_s=8, pass_condition="the vendor reset Pose2D topic and odom subscriber are discoverable; callback survival requires a separately controlled staging probe", block_code="ODOM_RESET_CALLBACK_CRASH"),
        TraceDiagnosticStep(step_id="BRINGUP_ENVIRONMENT", title="bringup environment contract", argv=("env",), timeout_s=5, pass_condition="all launch-required machine environment variables are present", block_code="BRINGUP_ENV_MISSING"),
    )
