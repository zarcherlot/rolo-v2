...
| FEAT-RKB-EPISODE-METADATA | PARTIAL | E2 | metadata-only Episode publish/load/rollback canary | `src/rolo/rkb/episodes.py`; `src/rolo/commands/lifecycle.py`; `scripts/rkb4_episode_canary.py`; `schemas/RKBEpisodeMetadata.schema.json` | `tests/test_rkb_episode.py` | 已完成离线 snapshot → typed query → Episode → 原子 latest/rollback，并接入 Probe orchestration；LanderPi identity → runtime → graph → app smoke 仍待目标机 SSH 恢复，不能提升为 STABLE |
