...
| FEAT-RKB-TYPED-READ-MODELS | PARTIAL | E2 | `robot.identity`; `os.runtime.status`; `hw.inventory.scan`; `middleware.*`; `app.executable.inspect`; `capability.get`; `state_safety.snapshot` | `src/rolo/rkb/read_models.py`; `src/rolo/rkb/query.py` | `tests/test_rkb_typed_queries.py`; RKB-1 compatibility tests | 离线 Bundle → snapshot → typed query → DiscoveryReport 闭环已通过；query 对 digest/fingerprint/freshness fail-closed；尚未完成固定目标机 RKB-2 真机 canary，state safety 仍是 UNKNOWN/观察投影 |
