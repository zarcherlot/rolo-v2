...
| FEAT-RKB-EPISODE-METADATA | PARTIAL | E3 | metadata-only Episode publish/load/rollback/query canary | `src/rolo/rkb/episodes.py`; `src/rolo/commands/lifecycle.py`; `scripts/rkb4_episode_canary.py`; `scripts/rkb4_fault_canary.py`; `scripts/rkb4_concurrency_canary.py`; `scripts/rkb4_migration_canary.py`; `schemas/RKBEpisodeMetadata.schema.json` | `tests/test_rkb_episode.py`; `docs/validation/RKB4_LANDERPI_SMOKE_20260903.json`; `docs/validation/RKB4_LANDERPI_POSTREBOOT_20260903.json`; `docs/validation/RKB4_LANDERPI_STORAGE_CANARY_20260903.json`; `docs/validation/RKB4_LANDERPI_MIGRATION_CANARY_20260903.json` | 固定 LanderPi 已完成操作员 reboot 后 fresh MHS → snapshot → Episode 只读 smoke；Docker 内 ROS2 graph 观察到 29 nodes/248 services/54 topics，并确认 `/robot_api`、`/rosapi`、`/rosbridge_websocket`、`/web_video_server` route presence；目标机已完成合成 snapshot 上的 fault recovery、跨进程并发发布与迁移 rollback；identity 已绑定已验证 fingerprint，runtime 可用，但 typed graph/state-safety 仍未进入 MHS snapshot，且 Episode 进程 kill-9 后持久化恢复尚未完成，因此保持 PARTIAL，仅允许 mentorpi metadata-only 灰度，不扩大平台或宣称真机闭环 |
