...
| FEAT-RKB-EPISODE-METADATA | PARTIAL | E3 | metadata-only Episode publish/load/rollback/query canary | `src/rolo/rkb/episodes.py`; `src/rolo/commands/lifecycle.py`; `scripts/rkb4_episode_canary.py`; `scripts/rkb4_fault_canary.py`; `schemas/RKBEpisodeMetadata.schema.json` | `tests/test_rkb_episode.py`; `docs/validation/RKB4_LANDERPI_SMOKE_20260903.json` | 固定 LanderPi 已完成 fresh MHS inspect/status/read → snapshot → Episode publish/query 只读 smoke；本地已补跨进程并发发布、指标合并和损坏 record 隔离；identity 已绑定已验证 fingerprint，runtime 可用，但 ROS2 graph 与 application route 为 UNKNOWN，因此保持 PARTIAL，仅允许 mentorpi metadata-only 灰度，不扩大平台或宣称真机闭环 |
