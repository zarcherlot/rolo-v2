...
该清单对应 `landerpi-mvp-journey` 发布候选（M7）。当前 MVP 功能切换为“地盘旋转调试”；所有离线步骤必须在干净 checkout 中可重复，真机步骤必须保存目标指纹和 artifact index。
...
- [x] `python scripts/mvp_release_gate.py --suite examples/chassis-rotation-10.json`
- [x] `examples/chassis-rotation-10.json` 覆盖 10 条旋转预检和 dry-run 用例。
...
后续使用足够的 120 秒窗口完成了完整 Probe（`hw=PARTIAL`、`linux=SUCCEEDED`、`ros=SUCCEEDED`），并修复了 bundle JSON round-trip digest 校验。新鲜 bundle 观察到 `/cmd_vel`、`/controller/cmd_vel` 和 `/odom*` 路由，旋转预检可进入 `READY_FOR_SUPERVISED_REVIEW`；这只是路由和里程计前置条件，`app.base.rotate` 仍是 deferred physical write，不能升级为 `agent_callable` capability。
...
- [ ] 在现场运行 `chassis-rotation-10`，10 条结果均可关联到独立 `run_id`、`case_id`、operation/evidence ID 和 artifact digest。
...
- [ ] 发布说明列出已知限制：无经授权的旋转写 capability 时 Trace 必须阻断，过期 catalog 不可消费，MVP 不支持 `UNATTENDED_REMOTE`。
