...
- [ ] 仅在现场安全员/调试工程师确认、用户任务明确、`SUPERVISED_FIELD_DEBUG` 和安全声明齐全时运行已注册实验写能力；默认保持只读。
...
- [ ] 发布说明列出已知限制：无目标绑定旋转写 Tool 时 Trace 必须阻断，过期 catalog 不可消费，MVP 不支持 `UNATTENDED_REMOTE`。
