...
def run(root: Path, device_prefix: str) -> dict[str, Any]:
    ...
    for candidate, provider in inventory.providers():
        ...
        observations.append({"device_id": candidate.manifest.device_id,
            "status": result.status.value, "manifest_digest": candidate.manifest.manifest_sha256,
            "result": result.model_dump(mode="json"), "limitations": candidate.manifest.limits})
    payload = {"schema_version": "rolo-mhs-readonly-canary/v1", "root": str(root),
        "device_prefix": device_prefix, "observations": observations, "write_requests": 0,
        "read_only": True, "limitations": ["offline fixture only", "no release authority", "no device writes"]}
